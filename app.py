...
app.config['MYSQL_PASSWORD'] = '1NN0v&te'
...
def login():
    if request.method == 'POST':
        ...
        canidate = request.form['password']
        ...
        if accpass is None:
            ...
            if sha256_crypt.verify(canidate, accpass):
                log_in_user(username)
                flash('You are now logged in.', 'success')
                return redirect(url_for('dashboard'))
            else:
                flash('Invald password', 'danger')
                return redirect(url_for('login'))
